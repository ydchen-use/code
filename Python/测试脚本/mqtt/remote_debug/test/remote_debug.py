...
def judge_req(service_num):
    ...
    try:
        ...
        if service_num == "4":
            ...
            tmp_send_msg["requestId"] = request_id
...
def get_user_input():
    ...
    try:
        ...
        #     #                     auto_suggest=AutoSuggestFromHistory(), completer=BaseCompleter)
        #     # user_input = user_input.strip().lower()  # 进行格式化
        #     # p = subprocess.Popen(user_input, stdout=subprocess.PIPE)
        #     # (output, err) = p.communicate()
        #     # os.system("ssh -p 22 kbdev@8.133.165.249")
        #     # tmp_method_name = num_to_method_name.get(user_input)  # 获取请求的服务名
        #     # send_msg = method_names.get(tmp_method_name)
        ...
        if user_input in req_one_time_list:
            # 获取发送信息
            request_id = str(uuid.uuid4()).replace("-", "")
            tmp_send_msg = method_names.get(num_to_method_name.get(str(user_input)))
            tmp_send_msg["requestId"] = request_id
            send_msg = tmp_send_msg
            if user_input == "8":
                print("请求已发送，请等待5秒，输入exit退出；登陆堡垒机，进行进一步操作！")
                time.sleep(5)
        else:
